...
if (typeof window.GlobalEvent == 'undefined' || !window.GlobalEvent) {
    ...
    window.GlobalEvent.CHANGE_ORIENTATION = "change_orientation";
    ...
    const Common = (function(ns){
        var _init = function(){
            setTimeout(function(){
                ScrollEvent.created();
                ResizeEvent.created();
                OrientationEvent.created();
                ns.ui.main.init();
                ns.content.main.init();
                ns.content.work.init(Swiper);
                ns.content.company.init();
                ns.content.list.init();
            },100);
        };
