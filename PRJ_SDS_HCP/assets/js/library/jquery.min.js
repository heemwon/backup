...
(function (E, A) {
  ...
  function ca(a, b, d, e) {
    if (c.isArray(b) && b.length)
      c.each(b, function (f, h) {
        d || Na.test(a)
          ? e(a, h)
          : ca(a + '[' + (typeof h === 'object' || c.isArray(h) ? f : '') + ']', h, d, e);
      });
    ...
    c = (function () {
      ...
      b.extend({
        ...
        uaMatch: function (i) {
          i = i.toLowerCase();
          i =
            M.exec(i) || g.exec(i) || j.exec(i) || (i.indexOf('compatible') < 0 && o.exec(i)) || [];
      ...
      if (a && typeof a === 'string')
        for (var b = (a || '').split(ga), d = 0, e = this.length; d < e; d++) {
          ...
          if (f.nodeType === 1)
            if (f.className) {
              for (
                var h = ' ' + f.className + ' ', k = f.className, l = 0, n = b.length;
                l < n;
                l++
              )
      ...
      if ((a && typeof a === 'string') || a === A)
        for (var b = (a || '').split(ga), d = 0, e = this.length; d < e; d++) {
          ...
          if (f.nodeType === 1 && f.className)
            if (a) {
              for (
                var h = (' ' + f.className + ' ').replace(qa, ' '), k = 0, l = b.length;
                k < l;
                k++
              )
                h = h.replace(' ' + b[k] + ' ', ' ');
        ...
        if (b) {
          ...
          if (c.nodeName(b, 'select')) {
            ...
            for (e = b ? e + 1 : f.length; h < e; h++) {
              ...
              if (
                k.selected &&
                (c.support.optDisabled ? !k.disabled : k.getAttribute('disabled') === null) &&
                (!k.parentNode.disabled || !c.nodeName(k.parentNode, 'optgroup'))
              ) {
                a = c(k).val();
                if (b) return a;
                d.push(a);
      ...
      if (q.length > 1 && s.exec(g))
        ...
        if (j) {
          t = m
            ...
            : l.find(
                q.pop(),
                q.length === 1 && (q[0] === '~' || q[0] === '+') && j.parentNode ? j.parentNode : j,
                R,
              );
  ...
  c.extend({
    ...
    cleanData: function (a) {
      for (
        var b, d, e = c.cache, f = c.event.special, h = c.support.deleteExpando, k = 0, l;
        (l = a[k]) != null;
        k++
      )
        if (!(l.nodeName && c.noData[l.nodeName.toLowerCase()]))
          if ((d = l[c.expando])) {
            if ((b = e[d]) && b.events)
              for (var n in b.events) f[n] ? c.event.remove(l, n) : c.removeEvent(l, n, b.handle);
  ...
  if (!c.support.opacity)
    c.cssHooks.opacity = {
      get: function (a, b) {
        return db.test((b && a.currentStyle ? a.currentStyle.filter : a.style.filter) || '')
          ...
          if (l && (k === 'height' || k === 'width')) {
            ...
            if (c.css(this, 'display') === 'inline' && c.css(this, 'float') === 'none')
              if (c.support.inlineBlockNeedsLayout)
                if (oa(this.nodeName) === 'inline') this.style.display = 'inline-block';
                else {
