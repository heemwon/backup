...
const onClickAccordion = (t) => {
  ...
  if (!currTarget.classList.contains('active')) {
    ...
    if (listActive) {
      window.scrollTo({ top:absoluteTop - listActive.clientHeight - headerHeight, left:0, behavior: "smooth" });
    } else {
      window.scrollTo({ top:absoluteTop - headerHeight, left:0, behavior: "smooth" });
    }
  } else {
...
// 마케팅 정보 수신 checkbox
const marketingTlt = document.querySelector("input[data-marketing-wrap]")
const marketing = document.querySelectorAll("input[data-marketing]")

if (marketingTlt) {
  marketingTlt.addEventListener("click", function(){
    if (marketingTlt.checked) {
  ...
  for(let i = 0; i < marketing.length; i++){
    window.addEventListener("change", function(){
      const check = document.querySelectorAll("input[data-marketing]:checked")
      if (check.length === 3){
        marketingTlt.checked = true
      } else {
        marketingTlt.checked = false
      }
    })
  }
}
...
if (checkWrap) {
  checkWrap.forEach((el) => {
    ...
    // 필수 항목 체크를 받아온다
    const required = el.querySelectorAll('input[data-option="required"]');
    ...
    const checkEvt = () => {
      ...
      checkList.forEach((data) => {
        ...
        // 필수항목 체크
        if (required.checked === false) {
          document.querySelector('.checked-wrap.font-body4').classList.add('active');
        } else {
          document.querySelector('.checked-wrap.font-body4').classList.remove('active');
        }
      });
    ...
    //헤더 체크에 이벤트 할당
    // checkAll.forEach((target) => {
    //   target.removeEventListener('click', checkHeader);
    // target.addEventListener('click', checkHeader);
    // }); 
  });
}
...
const architecture = document.querySelector(".architecture-icon")
if (main) {
  // main 로드 익터랙션
  ...
  const swiper = new Swiper('.mySwiper', {
    ...
    on: {
      slideChange: function (sw) {
        const offer1 = document.querySelector('.swiper-num3');
        const offer2 = document.querySelector('.swiper-num4');
        offer1.innerHTML = (sw.realIndex + 1);
        offer2.innerHTML = sw.loopedSlides;
      },
    },
    breakpoints: {
...
if (architecture) {
  // 아키텍쳐 아이콘 스와이프
  const swiper = new Swiper('.iconSwiper', {
    slidesPerView: 'auto',
    // spaceBetween: 11,
    // centeredSlides: false,
    loop: true,
...
const calcNavi = document.querySelector(".calulator .calc-navi")
const bottom = document.querySelector(".info-bottom-text")
if(calculator) {
  window.addEventListener("resize", function(){
    if(window.innerWidth >= 1320){
      window.addEventListener("scroll", function(){
        const target = document.querySelector(".calulator .calc-container")
          if(window.pageYOffset >= target.offsetTop && bottom.getBoundingClientRect().top > 1056) {
...
const foot = document.querySelector('.form');
if (foot) {
  // 이용약관 form 이벤트
  const form = document.getElementById('selectForm1');
  const data = document.querySelectorAll('[data-form]');

  form.addEventListener('change', function(){
    for(let i = 1; i <= data.length; i++){
      data[i-1].style.display = "none";
      if (this.value == i) {
        document.querySelector("[data-form='" + i + "']").style.display = "block";
        console.log()
      }
    }
  })
...
function familySite(){
  if(event.target.value === "samsung_SDS") {
    location.href = "https://www.samsungsds.com/kr/index.html"
  } else if(event.target.value === "cloud") {
    location.href = "https://www.samsungsds.com/kr/cloud/cloud.html"
  }
}